...
Currently the script supports **stationary** observations, i.e. when observations are made from a single position. (I am planning to add an extension for travelling observers.)
...
where $a_{\text{diff}}$ is the calculated dip (in radians) <br/>
$d = \sqrt{h \left( 2r + h \right)}$. <br/>
$h$ is the height of the observer (in meters). <br/>
